...
public class frames2 : MonoBehaviour {
    ...
    void Update()
    {
        PlayerPoints = pole.PlayerPoints;
        CompPoints = pole.CompPoints;
        if (Mathf.Abs(PlayerPoints - CompPoints) < 20)
        {
            _pos.position = new Vector2(_pos.position.x, _y + (PlayerPoints - CompPoints) / 100f);
            gameObject.transform.localScale = new Vector2(gameObject.transform.localScale.x, scaleY + (PlayerPoints - CompPoints) / 400f);
        }
